...
window.onload = function(){
	...
	var prg = {}; var texprg = {} ; var ps_prg = {};
	initShader();
	// create textures
	glnv.initTextures('../../lib/textures');
	glnv.addTexture('./images/smoke.png', 256, 256, 8);
	...
	// generate particles
	var busyflag = 0;
	var particle_model = mmglu.generateParticle(0);
	...
	(function(){
		...
		if (g.framerate_counter++%g.display_framerate_interval == 0) {
			displayFramerate();
			if ((g.framerate_counter%90)==1) {
			//var val = getLoadAvg();
			var value = 3.0*Math.sin(g.framerate_counter*0.002) - Math.sin(g.framerate_counter);
			drawColorBar(4.0-value);
			if ( busyflag == 0 && value < 2.0) {
				particle_model = mmglu.generateParticle(1); busyflag=1;
			} else if ( busyflag == 1 && value >= 2.0) {
				particle_model = mmglu.generateParticle(0); busyflag=0;
			}
			$('#colorbar_title').text($.sprintf(
				"Controller01 - Load Average (val=%.1f, threshold=2.0)", 4.0-value)
			);
			}
		}
		/// update xRot/yRot
	...
	function drawObjects() {
		// draw particles
		gl.useProgram(ps_prg);
		gl.disable(gl.DEPTH_TEST);
		gl.enable(gl.BLEND);
		gl.blendFunc(gl.SRC_ALPHA, gl.ONE);
		mmglu.drawParticle(ps_prg, [0.0, 0.0, 0.0], particle_model, m, pMatrix);
		gl.disable(gl.BLEND);
	...
	// initialize canvas
	function initCanvas() {
		gl.clearColor(0.1, 0.1, 0.7, 0.9); gl.clearDepth(1.0);
	...
	function initShader() {
		...
		var v_shader = glnv.createShader('raw', 'x-shader/x-vertex', mmglu.getParticleVertexShader());
		var f_shader = glnv.createShader('raw', 'x-shader/x-fragment', mmglu.getParticleFragmentShader());
		ps_prg = glnv.createProgram(v_shader, f_shader);
		mmglu.initParticleShader(ps_prg);
