...
function changePlaybackSpeed(sb) {
	g.playback_speed = sb.value;
}

// initialize ui
function initUI() {
	$(".switch-play").click(function(){
		var parent = $(this).parents('.switch');
		$('.switch-stop',parent).removeClass('selected');
		$(this).addClass('selected');
		g.stop_flag=0;
	});
	$(".switch-stop").click(function(){
		var parent = $(this).parents('.switch');
		$('.switch-play',parent).removeClass('selected');
		$(this).addClass('selected');
		g.stop_flag=1;
	});
	$('#switch-stop').removeClass('selected');
	$('#switch-play').addClass('selected');
	g.stop_flag=0;
	togglePerspective(0);
}
